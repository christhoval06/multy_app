...
import 'package:multiplication_app/models/table_overall_stats.dart';
import 'package:multiplication_app/services/objectbox_service.dart';
import 'package:multiplication_app/objectbox.g.dart';
...
class QuizAttempProvider extends ChangeNotifier {
  ...
  Future<List<TableOverallStats>> getAllTableOverallStats() async => _tableOverallStats.getAll();
  ...
  Future<void> loadStatistics() async {
    ...
    _allTableStats.sort((a, b) => a.tableNumber.compareTo(b.tableNumber)); // Ordenar por número de tabla
...
void updateTableOverallStats(QuizAttemp newAttempt) {
  ...
  if (existingStats == null) {
    ...
    if (newAttempt.totalQuestions > 0) {
      double newAttemptAccuracy = newAttempt.correctAnswers / newAttempt.totalQuestions;
      ...
      } else if (newAttemptAccuracy == currentBestAccuracy && newAttempt.correctAnswers > existingStats.bestScoreCorrect) {
        existingStats.bestScoreCorrect = newAttempt.correctAnswers;
        existingStats.bestScoreTotal = newAttempt.totalQuestions;
      }
    } else if (existingStats.bestScoreTotal == 0) { // Si el mejor actual es 0/0 y el nuevo también
        existingStats.bestScoreCorrect = newAttempt.correctAnswers;
        existingStats.bestScoreTotal = newAttempt.totalQuestions;
    }
  }

  _tableOverallStats.put(existingStats); // Guardar (o actualizar) las estadísticas agregadas
}
}
