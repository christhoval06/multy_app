...
import 'package:multiplication_app/models/quiz_models.dart'; 
...
class QuizQuestion {
  ...
  factory QuizQuestion.generate(int tableNumber, int multiplier) {
    ...
    final random = Random();
    Set<int> incorrectOptions = {}; // Usamos Set para evitar duplicados iniciales
    ...
    while (incorrectOptions.length < 3) {
      ...
      switch (strategy) {
        ...
        case 1: // Respuesta +/- número de tabla
             incorrect = correctAnswer + (random.nextBool() ? tableNumber : -tableNumber);
             break;
        case 2: // Respuesta +/- multiplicador
              incorrect = correctAnswer + (random.nextBool() ? multiplier : -multiplier);
              break;
        default: // Número completamente aleatorio cerca de la respuesta
              int range = max(10, (correctAnswer * 0.3).round()); // Rango del 30% o 10
